...
async fn admin_export_accounts(
    ...
    let all_accounts = account::export_accounts().map_err(|e| {
        (
            StatusCode::INTERNAL_SERVER_ERROR,
            Json(ErrorResponse { error: e }),
        )
    ...
    // Filter by requested account IDs
    let response: Vec<(String, String)> = all_accounts
        .into_iter()
        .filter(|(email, _)| payload.account_ids.iter().any(|id| id == email))
        .collect();
