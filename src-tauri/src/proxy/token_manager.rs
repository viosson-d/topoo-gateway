...
impl TokenManager {
    ...
    async fn get_token_internal(
        ...
        if let Some(ref pref_id) = preferred_id {
            ...
            {
                // [FIX] In Fixed Account mode, we MUST use the selected account.
                // Do NOT fallback to round-robin even if rate-limited.
                // The user explicitly selected this account, so they expect it to be used.
                // If it fails, they should see the error rather than silently switching.

                tracing::info!(
                    "🔒 [FIX #820] Using preferred account: {} (fixed mode) - Forcing usage regardless of rate limits",
                ...
                if now >= token.timestamp - 300 {
                    ...
                    match crate::modules::oauth::refresh_access_token(&token.refresh_token, None)
                        .await
                    {
                        Ok(token_response) => {
        ...
        for attempt in 0..total {
            ...
            if now >= token.timestamp - 300 {
                ...
                // 调用 OAuth 刷新 token
                match crate::modules::oauth::refresh_access_token(&token.refresh_token, None).await
                {
                    Ok(token_response) => {
    ...
    // / 保存 project_id 到账号文件
    pub async fn save_project_id(&self, account_id: &str, project_id: &str) -> Result<(), String> {
        let path = if let Some(entry) = self.tokens.get(account_id) {
            entry.account_path.clone()
        } else {
            self.data_dir
    ...
    pub fn get_account_id_by_email(&self, email: &str) -> Option<String> {
        for entry in self.tokens.iter() {
            if entry.value().email == email {
                return Some(entry.value().account_id.clone());
            }
        }
        None
    }
    ...
    pub async fn set_forbidden(&self, account_id: &str, reason: &str) -> Result<(), String> {
        ...
        content["is_forbidden"] = serde_json::Value::Bool(true);
        content["disabled_reason"] = serde_json::Value::String(truncate_reason(reason, 800));
    ...
    pub async fn set_validation_block_public(
        ...
        content["validation_blocked_until"] = serde_json::Value::Number(until.into());
        content["disabled_reason"] = serde_json::Value::String(truncate_reason(reason, 800));
    ...
    pub async fn set_preferred_account(&self, account_id: Option<String>) {
        ...
        if let Some(ref id) = account_id {
            // [FIX] Clear rate limit record for the preferred account to ensure immediate availability
            // This fixes the issue where switching back to a previously rate-limited account would fail silently
            if self.rate_limit_tracker.clear(id) {
                tracing::info!(
                    "🔒 [FIX #820] Fixed account mode enabled: {}, rate limit cleared",
                    id
                );
            } else {
