...
/// 解包响应(提取 response 字段,同时保留 usageMetadata)
pub fn unwrap_response(response: &Value) -> Value {
    if let Some(inner_response) = response.get("response") {
        let mut unwrapped = inner_response.clone();

        // [FIX] Preserve usageMetadata from the outer response
        // Gemini v1internal wraps the response, but usageMetadata is at the top level
        // We need to move it into the unwrapped response so monitor middleware can parse it
        if let Some(usage_metadata) = response.get("usageMetadata") {
            if let Some(obj) = unwrapped.as_object_mut() {
                obj.insert("usageMetadata".to_string(), usage_metadata.clone());
            }
        }
