...
pub async fn start_oauth_flow(
    ...
    let code =
        match tokio::time::timeout(tokio::time::Duration::from_secs(300), code_rx.recv()).await {
            Ok(Some(Ok(code))) => code,
            Ok(Some(Err(e))) => return Err(e),
            Ok(None) => return Err("OAuth flow channel closed unexpectedly".to_string()),
            Err(_) => return Err("OAuth flow timed out after 5 minutes".to_string()),
        };
