...
/// 验证并修复账号索引（应用启动时调用）
pub fn verify_and_repair_index() -> Result<(), String> {
    crate::modules::logger::log_info("[Index] Starting index verification and repair...");

    let accounts_dir = get_accounts_dir()?;
    let mut index = load_account_index()?;
    let original_count = index.accounts.len();

    // 扫描所有账号文件
    let entries =
        fs::read_dir(&accounts_dir).map_err(|e| format!("Failed to read accounts dir: {}", e))?;
                ...
                if !index.accounts.iter().any(|a| a.id == account_id) {
                    ...
                    match load_account(account_id) {
                        Ok(account) => {
                            ...
                            added_count += 1;
                            crate::modules::logger::log_info(&format!(
                                "[Index] Repaired: added {} ({})",
                                account.email, account_id
                            ));
                        }
                        Err(e) => {
                            crate::modules::logger::log_warn(&format!(
                                "[Index] Failed to load account {}: {}",
                                account_id, e
                            ));
                        }
                    }
    ...
    if repaired {
        ...
        crate::modules::logger::log_info(&format!(
            "[Index] Repair completed: {} accounts added (total: {} -> {})",
            added_count,
            original_count,
            index.accounts.len()
        ));
        ...
        crate::modules::logger::log_info(&format!(
            "[Index] Verification passed: {} accounts, no repair needed",
            index.accounts.len()
        ));
    }
    ...
    for mut account in accounts {
        ...
        if changed {
            if let Err(e) = save_account(&account) {
                crate::modules::logger::log_error(&format!(
                    "Failed to reset forbidden flag for {}: {}",
                    account.email, e
                ));
            } else {
                count += 1;
...
pub async fn switch_account(
    ...
    // [DEBUG] 添加详细的调试信息
    crate::modules::logger::log_info(&format!(
        "[Switch] Loaded index with {} accounts, looking for ID: {}",
        index.accounts.len(),
        account_id
    ));
    ...
    if !index.accounts.iter().any(|s| s.id == account_id) {
        // [DEBUG] 添加更详细的错误信息
        let available_ids: Vec<String> = index
            .accounts
            .iter()
    ...
    let fresh_token = oauth::ensure_fresh_token(&account.token, None)
        .await
...
pub async fn fetch_quota_with_retry(account: &mut Account) -> crate::error::AppResult<QuotaData> {
    ...
    let token = match oauth::ensure_fresh_token(&account.token, None).await {
        Ok(t) => t,
        Err(e) => {
    ...
    if token.access_token != account.token.access_token {
        ...
        {
            match oauth::get_user_info(&token.access_token, None).await {
    ...
    if account.name.is_none() || account.name.as_ref().map_or(false, |n| n.trim().is_empty()) {
        ...
        // Use updated token
        match oauth::get_user_info(&account.token.access_token, None).await {
            Ok(user_info) => {
        ...
        if let Some(status) = e.status() {
            if status == StatusCode::UNAUTHORIZED {
                ...
                let name = if account.name.is_none()
                    || account.name.as_ref().map_or(false, |n| n.trim().is_empty())
                {
                    match oauth::get_user_info(&token_res.access_token, None).await {
                        Ok(user_info) => user_info.get_display_name(),
                        Err(_) => None,
                    }
                } else {
                ...
                let retry_result: crate::error::AppResult<(QuotaData, Option<String>)> =
                    modules::fetch_quota(&new_token.access_token, &account.email, None).await;
